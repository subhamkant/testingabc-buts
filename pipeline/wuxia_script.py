...
- COMBAT / CONFRONTATION / DIALOGUE-BETWEEN-CHARACTERS beats: the "prompt" MUST be a TWO-SHOT (or group shot) with ALL involved characters named and FULLY VISIBLE in one frame, using EXPLICIT left/right spatial tokens — say which character is "on the left side" and which is "on the right side", with clear space between them, and who does what to whom. NEVER a solo close-up for a fight beat, never let positioning be implicit.
- DUEL CAMERA PROTOCOL: action beats are filmed from a WIDE SIDE-PROFILE angle (camera perpendicular to the line between the fighters — "side view, both fighters seen in profile") — NEVER a straight-on punch toward the camera (it blocks faces and confuses spatial tracking). Add: "two distinct fighters standing well apart, completely separate bodies, clear empty space between them, no overlapping limbs, anatomically correct hands with five fingers each, natural human anatomy". Between action beats, insert CLOSE-UP REACTION shots of ONE named character's face (these may be tight close-ups — that is where facial detail lives).
