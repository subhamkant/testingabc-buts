...
def _build_wuxia_notebook(run_config: dict[str, Any], ltx_script: str) -> dict[str, Any]:
    ...
    cell1 = "\n".join([
        ...
        "",
        "import subprocess as _sp, torch, os, sys, json, base64",
        "",
        "# --- -9 SIGKILL fix (2026-07-20): the OOM-killer nukes the mp4-writer's",
        "# ffmpeg fork() when Wan's ~18GB cpu-offload footprint would be briefly",
        "# doubled by fork-before-exec. overcommit_memory=1 lets the COW fork",
        "# succeed; an 8GB swapfile absorbs any residual host-RAM spike. Kaggle",
        "# kernels run as root, so both are permitted. ---",
        "_sp.run('sysctl -w vm.overcommit_memory=1', shell=True, capture_output=True)",
        "if not _sp.run('swapon --show', shell=True, capture_output=True, text=True).stdout.strip():",
        "    _sp.run('fallocate -l 8G /swapfile && chmod 600 /swapfile && mkswap /swapfile && swapon /swapfile', shell=True, capture_output=True)",
        "print('overcommit=' + _sp.run('cat /proc/sys/vm/overcommit_memory', shell=True, capture_output=True, text=True).stdout.strip(), '| swap:', bool(_sp.run('swapon --show', shell=True, capture_output=True, text=True).stdout.strip()))",
        "",
